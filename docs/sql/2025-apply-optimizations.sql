-- ═══════════════════════════════════════════════════════════════
-- 🚀 QUICK DEPLOYMENT SCRIPT - Apply All Optimizations
-- ═══════════════════════════════════════════════════════════════
-- Run this script in Supabase SQL Editor to apply all optimizations
-- Estimated execution time: 30-60 seconds (depending on data size)
-- ═══════════════════════════════════════════════════════════════
...
-- Step 2: Create all critical indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answers_category_id 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answers_category_language 
  ON answers(category_id, language) 
  WHERE category_id IS NOT NULL AND language IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answers_category_country 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answers_selected_code 
  ON answers(selected_code) 
  WHERE selected_code IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_name_search 
  ON codes USING gin(name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_categories_category_code 
  ON codes_categories(category_id, code_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_categories_code_category 
  ON codes_categories(code_id, category_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answer_codes_answer_id 
  ON answer_codes(answer_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_answer_codes_code_id 
...
WHERE routine_schema = 'public'
  AND routine_name IN ('get_category_stats', 'get_filter_options');
...
-- Next: Update frontend code as described in optimization guide.
-- ═══════════════════════════════════════════════════════════════
